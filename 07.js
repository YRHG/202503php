...
$('#title').hide(1); //yincangyuansu 隐藏元素
// ID 选择器
// document.getElementById('title').sthle.background-color = '#F4040';
$("header").css("background-color", "#F4040"); //设置元素样式。为啥title是反引号 header是双引号 忘了得问一下
// 类选择器 
...
// 布尔是啥意思：是真假的意思 一种判断关系 真的就继续假的就重来
...
// 事件操作
// .on 方法用于绑定事件, 第一个参数是事件名称, 第二个参数是事件处理函数。这里是选择btn2元素绑定事件“单击鼠标”开始然后回调函数，类似js的监听 逻辑更直接。
let btn2 = $("#btn-2");
btn2.on("click", function () {
    alert("Hello World!哈喽不哈喽要看回调函数让不让你往下继续执行");
});
...
// 不如让类似flash的动画外置制作功能直接代替 并给动画赋予标签和值的能力。网页交互才能更好呀
let box = $("#box");
// box.hide('slow');
box.hide();
box.show('slow');
